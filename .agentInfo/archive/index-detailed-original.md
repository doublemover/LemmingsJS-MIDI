# Archived Detailed Index (Pre-Reorg)

This snapshot preserves the prior index content. Paths reference `notes/` as of
capture; archived placeholders now live under `archive/notes/`.
...
- **level-format, l3, doc**: [notes/l3-level-format.md](notes/l3-level-format.md) - `docs/camanis/lemmings_3_level_file_format.md` outlines the Lemmings┬á3 header and object tables with extra lemming and enemy fields.
...
- **level-writing**: [notes/level-writer.md](notes/level-writer.md) - js/LevelWriter.js converts a level object back into the original 2048ΓÇæbyte binary format used by ...
...
- **stage, canvas, input**: [notes/stage.md](notes/stage.md) - HUD panel centers at the bottom (4├ù scale) and bottom anchoring keeps the level flush with the panel. `updateViewPoint` preserves cursor position when zooming; see `stage.updateStageSize.test.js` and `stage.updateviewpoint.test.js`.
