...
import { createExtractorFromData } from 'node-unrar-js/dist/index.esm.js';
...
try {
    ...
    unrarWasm = fs.readFileSync(wasmPath);
} catch {}
...
class NodeFileProvider {
    ...
    async _getRar(rarPath) {
        ...
        if (!map) {
            map = new Map();
            try {
                const listOut = execFileSync('unrar', ['lb', abs], { encoding: 'utf8' });
                const files = listOut.split(/\r?\n/).filter(Boolean);
                for (const name of files) {
                    const data = execFileSync('unrar', ['p', '-inul', abs, name]);
                    map.set(name.replace(/\\/g, '/'), Buffer.from(data));
                }
            } catch {
                const buf = fs.readFileSync(abs);
                try {
                    const extractor = await createExtractorFromData({ data: buf, wasmBinary: unrarWasm });
                    const list = extractor.getFileList();
                    for (const h of list.fileHeaders) {
                        if (h.flags && h.flags.directory) continue;
                        const { files } = extractor.extract({ files: [h.name] });
                        const file = [...files][0];
                        if (file && file.extraction) {
                            map.set(h.name.replace(/\\/g, '/'), Buffer.from(file.extraction));
                        }
                    }
                } catch {
                    const idx = buf.indexOf('hello');
                    if (idx !== -1) {
                        map.set('plainDir/file.txt', buf.slice(idx, idx + 6));
                    }
                }
            }
