...
const EXTRA_LEMMING_AMOUNT = 0; // used to multiply the number of lemmings spawned, 2+

class LemmingManager {
    static minimapDots = new Set();
    static lemmings = new Set();
    static log = null;
    constructor(level, lemmingsSprite, triggerManager, gameVictoryCondition, masks, particleTable) {
        ...
        LemmingManager.lemmings = new Set();
        LemmingManager.minimapDots = new Set();
        this.level = level;
    ...
    tick() {
        this.addNewLemmings();
        const lems = LemmingManager.lemmings;
        const count = lems.size;
        ...
        if (this.miniMap && ((++this.mmTickCounter % 10) === 0)) {
            const dots = LemmingManager.minimapDots;
            dots.clear();
            for (const lem of lems) {
    ...
    addLemming(x, y) {
        const lem = new Lemmings.Lemming(x, y, LemmingManager.lemmings.size);
        this.setLemmingState(lem, Lemmings.LemmingStateType.FALLING);
        LemmingManager.lemmings.add(lem);
        if (EXTRA_LEMMING_AMOUNT > 0) {
            for (let i = 0; i < 1*EXTRA_LEMMING_AMOUNT; i++) {
                const lem2 = new Lemmings.Lemming(x, y, LemmingManager.lemmings.size);
                this.setLemmingState(lem2, Lemmings.LemmingStateType.FALLING);
                LemmingManager.lemmings.add(lem2);
            }
        }
    }
    ...
    render(gameDisplay) { 
        for (const lem of LemmingManager.lemmings) { 
    ...
    getLemming(id) { 
        return [...LemmingManager.lemmings][id]; 
    }

    getLemmings() { 
        return LemmingManager.lemmings; 
    }
    ...
    getLemmingsInMask(mask, x, y) {
        ...
        for (let i = 0, l = lems.length; i < l; ++i) {
            ...
            const lx = val.x;
            const ly = val.y;
            if (lx > left && lx < right && ly > top && ly < bottom) out.add(val);
        }
        return out;
    }
    ...
    dispose() {
        LemmingManager.minimapDots = null;
        LemmingManager.lemmings = null;
        this.level = null;
